class ListMath:
    def __init__(self, lst=None):
        ...
        self.lst = list(filter(lambda x: type(x) in (int, float), self.lst))
        self.lst_math = self.lst

    def __add__(self, other):
        if not isinstance(other, (int, float)):
            raise ArithmeticError("Вторым операндом должно быть целое или вещественное "
                                  "число")
        arg = [i + other for i in self.lst]
        return ListMath(arg)
